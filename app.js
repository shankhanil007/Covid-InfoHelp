...
app.get("/world", function (req, res) {
  var c = 0,
    ...
    r = 0,
    d = 0;
  request("https://corona.lmao.ninja/v2/countries", function (
    error,
    response,
    body
  ) {
...
app.post("/news", function (req, res) {
  ...
  var url =
    ...
    pla +
    "&sxsrf=ALeKk02SupK-SO625SAtNAmqA5CHUj5xjg:1586447007701&source=lnms&tbm=nws&sa=X&ved=2ahUKEwikieXS19voAhXAxzgGHV5bCcQQ_AUoAXoECBwQAw&biw=1536&bih=535";
  ...
  (async () => {
    ...
    const desc = await page.evaluate(() =>
      Array.from(document.querySelectorAll("div.st")).map((text) =>
        text.innerText.trim()
      )
    );

    var data = await page.evaluate(() =>
      Array.from(document.querySelectorAll("div.g")).map((compact) => ({
        headline: compact.querySelector("h3").innerText.trim(),
        img:
          compact.querySelector("img") === null
            ? "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6c/No_image_3x4.svg/1280px-No_image_3x4.svg.png"
            : compact.querySelector("img.th.BbeB2d").src,
        url: compact.querySelector("h3.r.dO0Ag>a").href,
        source: compact
          .querySelector("div.gG0TJc>div.dhIWPd>span.xQ82C.e8fRJf")
          .innerText.trim(),
        time: compact
          .querySelector("div.gG0TJc>div.dhIWPd>span.f.nsa.fwzPFf")
          .innerText.trim(),
...
app.get("/info", function (req, res) {
  ...
  var s5 = [];
  var s6 = [];
  request("https://api.covid19india.org/resources/resources.json", function (
    error,
    response,
    body
  ) {
...
app.get("/:id/new/seller", isLoggedIn, function (req, res) {
  Seller.findOne({ seller_id: req.params.id })
    .populate("newOrder")
    .exec(function (err, details) {
      if (err) console.log(err);
        ...
        if (details != undefined) {
          ...
          res.redirect("/groceries");
        } else res.render("newseller.ejs", { user: req.user });
      }
    });
});
    ...
    if (err) console.log(err);
      ...
      NewOrder.create(req.body.order, function (err, details) {
        if (err) {
          ...
          req.flash(
            "success",
            "Successfully placed order. You will recieve notification regarding delivery details on your phone no. "
          );
